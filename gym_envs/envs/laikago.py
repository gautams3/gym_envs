...
class LaikagoEnv(MujocoEnv, EzPickle):
  ...
  def step(self, action):
    ...
    # Orientation
    orientation_cost = self._orientation_cost_weight * quat_dist(curr_state[3:7], np.array([0.5, 0.5, 0.5, 0.5]))
...
class Laikagov2Env(MujocoEnv, EzPickle):
  ...
  def reset_obstacle(self, step_height=None, step_location = None):
    ...
    if self._path_to_trajs is not None:
  ...
  def get_augmented_observation(self):
    ...
    augmented_obs['overlay_time'] = np.array([float(self.overlay_step_number)])
  ...
  def calculate_dense_reward(self, prev_state, curr_state, action):
    ...
    self.orientation_cost = self._orientation_cost_weight * quat_dist(curr_state[3:7], np.array([0.5, 0.5, 0.5, 0.5]))
